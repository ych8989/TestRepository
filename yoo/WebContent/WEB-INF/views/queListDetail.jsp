		...
		$.ajax({
			...
			success : function(data) {
				if (data.result == "success") {
					console.log("success");
					location.href = "queListUpdate?bno=${board.bno}"
				} else {
					$("#bpassword").val("");
	...
	function handleBtnDelete() {
		...
		$.ajax({
			...
			success : function(data) {
				if (data.result == "success") {
					console.log("success");
					location.href = "queListDelete?bno=${board.bno}"
				} else {
					$("#bpassword").val("");
					$("#bpassword").attr("placeholder", "비밀번호가 다릅니다.");
					$("#bpassword").css("border-color", "red");
	...
	<form method="post" style="padding: 0px 20px"
		...
		<div class="form-group">
			<div class="input-group">
				...
				</span> <input type="date" class="form-control" placeholder="작성 일시"
		...
		<div class="form-group">
			<div class="input-group">
				...
				</span> <a class="form-control" href="viewimage?bno=${board.bno }">${board.boriginalfilename}</a>
			</div>
		</div>
		...
		<a href="queList" class="btn btn-success">목록</a> 
		<input onclick="handleBtnUpdate()" type="button" 
				class="btn btn-warning" value="수정" /> 
		<input onclick="handleBtnDelete()" type="button"
