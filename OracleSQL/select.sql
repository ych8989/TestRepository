...
--bwriter 가 '홍길동'인 모든 컬럼의 값을 가져오시오 
select *
from board
where bwriter='홍길동';

--bwriter가 홍길동인 bno, btitle ,bwriter를 가져 오시오
select bno, btitle, bwriter
from board
where bwriter='이정수';

--bno 가 1에서 10 사이의 게시물을 가져오시오 
select bno, btitle, bwriter
from board
where bno between 1 and 30;

-- bwriter가 '홍'으로 포함하는 게시물의 bno,btitle,bwriter를 가져오시오
select bno, btitle, bwriter
from board
where bwriter like '%홍%';

-- 게시물의 제목 중에 '자바' 가 포함되어있는 게시물의 번호,제목,글쓴이를 가져오시오
select bno, btitle, bwriter
from board
where btitle like '%자바%';

-- 게시물의 제목,내용 중에 '자바' 가 포함되어있는 게시물의 번호,제목,글쓴이를 가져오시
...
--글쓴이를 기준으로 1차 절렬하고(올림) 쓴날짜를 기준으로 2차 정렬(내림)하시오
select *
from board
